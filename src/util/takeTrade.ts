...
// NOTE Use the Escrow Account Pubkey from UI to get the data from the Escrow Account.
// Then we decode the Escrow Account data. Afterwards, we take this decoded data
// plus Bob's data to send the transaction.
export const takeTrade = async (
  ...
  const takerAccount = new Account(
    privateKeyByteArray.split(",").map((s) => parseInt(s))
  );
  const escrowAccountPubkey = new PublicKey(escrowAccountAddressString);
  const takerXTokenAccountPubkey = new PublicKey(
    takerXTokenAccountAddressString
  );
  const takerYTokenAccountPubkey = new PublicKey(
  ...
  try {
    encodedEscrowState = (await connection.getAccountInfo(
      escrowAccountPubkey,
      "singleGossip"
    ))!.data;
  } catch (err) {
  ...
  const exchangeInstruction = new TransactionInstruction({
    programId,
    data: Buffer.from(
      Uint8Array.of(1, ...new BN(takerExpectedXTokenAmount).toArray("le", 8))
    ),
    keys: [
